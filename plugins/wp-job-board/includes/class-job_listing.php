...
class WP_Job_Board_Job_Listing {
	...
	public static function init() {
		...
		// restrict
		add_action( 'wp-job-board-job_listing-query-args', array( __CLASS__, 'job_restrict_listing_query_args'), 100, 2 );
		add_action( 'wp-job-board-job_listing-filter-query', array( __CLASS__, 'job_restrict_listing_query'), 100, 2 );
	}
	...
	public static function check_view_job_detail() {
		global $post;
		$restrict_type = wp_job_board_get_option('job_restrict_type', '');
		$view = wp_job_board_get_option('job_restrict_detail', 'all');
		...
		return apply_filters('wp-job-board-check-view-job-detail', $return, $post);
	}
	...
	public static function job_restrict_listing_query($query, $filter_params) {
		...
		return apply_filters('wp-job-board-check-view-job-listing-query', $query);
	}
	...
	public static function job_restrict_listing_query_args($query_args, $filter_params) {
		...
		if ( $restrict_type == 'view' ) {
			$view = wp_job_board_get_option('job_restrict_listing', 'all');
		...
		return apply_filters('wp-job-board-check-view-job_listing-listing-query-args', $query_args);
	}
