...
class WP_Job_Board_Post_Type_Candidate {
	...
	public static function save_post($post_id, $post) {
		if ( $post->post_type === 'candidate' ) {
			...
			WP_Job_Board_Candidate::update_post_meta($post_id, 'description', $description);
			$title = get_post_field('post_title', $post_id);
			WP_Job_Board_Candidate::update_post_meta($post_id, 'title', $title);
		}
	}
	...
	public static function fields( array $metaboxes ) {
		...
		$option = wp_job_board_get_option('candidate_field_email');
		...
		$option = wp_job_board_get_option('candidate_field_show_profile');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Show my profile', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('candidate_field_founded_date');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Date of Birth', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('candidate_field_phone');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
		...
		$option = wp_job_board_get_option('candidate_field_urgent');
		$option = isset($option[0]) ? $option[0] : array();
		$fields[] = array(
			'name'              => !empty($option['title']) ? $option['title'] : __( 'Urgent Candidate', 'wp-job-board' ),
			'id'                => self::$prefix . 'urgent',
			'type'              => 'checkbox',
			'description'		=> !empty($option['description']) ? $option['description'] : __( 'Urgent candidate will be sticky during searches, and can be styled differently.', 'wp-job-board' )
		);

		$option = wp_job_board_get_option('candidate_field_featured');
		$option = isset($option[0]) ? $option[0] : array();
		$fields[] = array(
			'name'              => !empty($option['title']) ? $option['title'] : __( 'Featured Candidate', 'wp-job-board' ),
			'id'                => self::$prefix . 'featured',
			'type'              => 'checkbox',
			'description'		=> !empty($option['description']) ? $option['description'] : __( 'Featured candidate will be sticky during searches, and can be styled differently.', 'wp-job-board' )
		);

		$option = wp_job_board_get_option('candidate_field_job_title');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Job Title', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('candidate_field_salary');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : sprintf(__( 'Salary (%s)', 'wp-job-board' ), $currency_symbol),
		...
		$option = wp_job_board_get_option('candidate_field_salary_type');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Salary Type', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('candidate_field_portfolio_photos');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Portfolio Photos', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('candidate_field_cv_attachment');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'CV Attachment', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('candidate_field_video_url');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
		...
		$option = wp_job_board_get_option('candidate_field_socials');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields[] = array(
		...
		$option = wp_job_board_get_option('candidate_field_address');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
		...
		$option = wp_job_board_get_option('candidate_field_map_location');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields[] = array(
		...
		$fields = array();
		$option = wp_job_board_get_option('candidate_field_education');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields[] = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Education', 'wp-job-board' ),
		...
		$fields = array();
		$option = wp_job_board_get_option('candidate_field_experience');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields[] = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Experience', 'wp-job-board' ),
		...
		$fields = array();
		$option = wp_job_board_get_option('candidate_field_award');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields[] = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Award', 'wp-job-board' ),
		...
		$fields = array();
		$option = wp_job_board_get_option('candidate_field_skill');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields[] = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Skill', 'wp-job-board' ),
	...
	public static function fields_front( array $metaboxes ) {
		if ( ! is_admin() ) {
			...
			$option = wp_job_board_get_option('candidate_field_profile_url');
			$option = isset($option[0]) ? $option[0] : array();
			if ( empty($option['enable']) || $option['enable'] == 'on' ) {
				$field = array(
			...
			$option = wp_job_board_get_option('candidate_field_featured_image');
			$option = isset($option[0]) ? $option[0] : array();
			if ( empty($option['enable']) || $option['enable'] == 'on' ) {
				$field = array(
					'name'              => !empty($option['title']) ? $option['title'] : __( 'Featured Image', 'wp-job-board' ),
			...
			$option = wp_job_board_get_option('candidate_field_title');
			$option = isset($option[0]) ? $option[0] : array();
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Full Name', 'wp-job-board' ),
			...
			$option = wp_job_board_get_option('candidate_field_category');
			$option = isset($option[0]) ? $option[0] : array();
			if ( empty($option['enable']) || $option['enable'] == 'on' ) {
				$field = array(
			...
			$option = wp_job_board_get_option('candidate_field_description');
			$option = isset($option[0]) ? $option[0] : array();
			$field = array(
			...
			$option = wp_job_board_get_option('candidate_field_location');
			$option = isset($option[0]) ? $option[0] : array();
			if ( empty($option['enable']) || $option['enable'] == 'on' ) {
				$field = array(
					'name'      		=> !empty($option['title']) ? $option['title'] : __( 'Location', 'wp-job-board' ),
			...
			$option = wp_job_board_get_option('candidate_field_tag');
			$option = isset($option[0]) ? $option[0] : array();
			if ( empty($option['enable']) || $option['enable'] == 'on' ) {
				$field = array(
					'name'      		=> !empty($option['title']) ? $option['title'] : __( 'Tags', 'wp-job-board' ),
					'id'        		=> self::$prefix . 'tags',
			...
			$fields = apply_filters( 'wp-job-board-candidate-fields-front', $fields );
	...
	public static function fields_resume_front( array $metaboxes ) {
		if ( ! is_admin() ) {
			...
			$fields = apply_filters( 'wp-job-board-candidate-fields-resume-front', $fields );
