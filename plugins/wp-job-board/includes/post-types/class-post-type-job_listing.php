...
class WP_Job_Board_Post_Type_Job_Listing {
	...
	public static function init() {
		...
		// Ajax endpoints.
		add_action( 'wjb_ajax_wp_job_board_ajax_remove_job',  array(__CLASS__,'process_remove_job') );
		add_action( 'wjb_ajax_wp_job_board_ajax_mark_filled_job',  array(__CLASS__,'process_mark_filled_job') );
		add_action( 'wjb_ajax_wp_job_board_ajax_mark_not_filled_job',  array(__CLASS__,'process_mark_not_filled_job') );
	...
	public static function process_mark_filled_job() {
		...
		if ( ! $is_allowed ) {
	        $return = array( 'status' => false, 'msg' => esc_html__('You can not mark this job as \'filled\'.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		$return = array( 'status' => true, 'msg' => esc_html__('Job has been successfully marked as \'filled\'.', 'wp-job-board'), 'title' => esc_html__('Mark not filled', 'wp-job-board'), 'nonce' => wp_create_nonce( 'wp-job-board-mark-not-filled-nonce' ), 'icon_class' => 'fa fa-lock',
	...
	public static function process_mark_not_filled_job() {
		...
		if ( ! is_user_logged_in() ) {
	        $return = array( 'status' => false, 'msg' => esc_html__('Please login to mark job as \'not filled\'', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		$job_id = empty( $_POST['job_id'] ) ? false : intval( $_POST['job_id'] );
		...
		if ( ! $is_allowed ) {
	        $return = array( 'status' => false, 'msg' => esc_html__('You can\'t mark this job as \'not filled\'.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		$return = array( 'status' => true, 'msg' => esc_html__('Job has been successfully marked as \'not filled\'.', 'wp-job-board'), 'title' => esc_html__('Mark filled', 'wp-job-board'), 'nonce' => wp_create_nonce( 'wp-job-board-mark-filled-nonce' ), 'icon_class' => 'fa fa-unlock', 'label' => '' );
		$return = apply_filters('wp-job-board-mark-filled-job-return', $return);
	...
	public static function fields( array $metaboxes ) {
		...
		$option = wp_job_board_get_option('job_field_application_deadline_date');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Application Deadline Date', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('job_field_apply_url');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'External URL for Apply Job', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('job_field_apply_email');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Job Apply Email', 'wp-job-board' ),
		...
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Min. Salary', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('job_field_max_salary');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : __( 'Max. Salary', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('job_field_salary_type');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
		...
		$option = isset($option[0]) ? $option[0] : array();
		$field = array(
			'name'              => !empty($option['title']) ? $option['title'] : __( 'Featured', 'wp-job-board' ),
			'id'                => self::$prefix . 'featured',
			'type'              => 'checkbox',
			'description'		=> !empty($option['description']) ? $option['description'] : __( 'Featured jobs will be sticky during searches, and can be styled differently.', 'wp-job-board' )
		);
		if ( !empty($option['placeholder']) ) {
			$field['attributes'] = array(
				'placeholder' => $option['placeholder']
		...
		$option = isset($option[0]) ? $option[0] : array();
		$field = array(
			'name'              => !empty($option['title']) ? $option['title'] : __( 'Urgent Job', 'wp-job-board' ),
			'id'                => self::$prefix . 'urgent',
			'type'              => 'checkbox',
			'description'		=> !empty($option['description']) ? $option['description'] : __( 'Urgent jobs will be sticky during searches, and can be styled differently.', 'wp-job-board' )
		);
		if ( !empty($option['placeholder']) ) {
			$field['attributes'] = array(
				'placeholder' => $option['placeholder']
		...
		$option = wp_job_board_get_option('job_field_filled');
		$option = isset($option[0]) ? $option[0] : array();
		$fields[] = array(
			'name'              => !empty($option['title']) ? $option['title'] : __( 'Filled', 'wp-job-board' ),
			'id'                => self::$prefix . 'filled',
			'type'              => 'checkbox',
			'description'		=> !empty($option['description']) ? $option['description'] : __( 'Filled listings will no longer accept applications.', 'wp-job-board' )
		);
		if ( !empty($option['placeholder']) ) {
			$field['attributes'] = array(
				'placeholder' => $option['placeholder']
		...
		$option = wp_job_board_get_option('job_field_address');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
