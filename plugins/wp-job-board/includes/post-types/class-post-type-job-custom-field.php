...
class WP_Job_Board_Post_Type_Job_Custom_Fields {
	...
	public static function init() {
		...
		self::$post_types =  array(
	  	...
	  	add_filter( 'wp-job-board-candidate-fields-resume-front', array( __CLASS__, 'add_fields_front_candidate_resume' ) );
	}
	...
	public static function fields( array $metaboxes, $post_type, $prefix ) {
		...
		$more_fields = [];
		if ( $post_type == 'candidate_cfield' ) {
			$more_fields[] = array(
				'name' => esc_html__( 'Show in', 'wp-job-board' ),
				'id'   => $prefix."show_in",
				'type' => 'select',
				'options' => apply_filters('wp-job-board-custom-field-show-in-options', array(
					'profile' => esc_html__( 'Profile Form', 'wp-job-board' ),
					'resume' => esc_html__( 'Resume Form', 'wp-job-board' ),
				)),
				'default' => 'profile'
			);
		}
	...
	public static function get_custom_fields($post_type = 'job_cfield', $show_in = '') {
		switch ($post_type) {
        ...
        if ( $post_type == 'candidate_cfield' && !empty($show_in) ) {

        	if ( $show_in == 'resume' ) {
	        	$args['meta_query'] = array(
	        		array(
	        			'key'       => WP_JOB_BOARD_CANDIDATE_CUSTOM_FIELD_PREFIX . 'show_in',
						'value'     => $show_in,
						'compare'   => '==',
	        		)
	        	);
	...
	public static function add_fields_front_job_listing($fields) {
		...
		$cfields = self::generate_fields($post_type, $prefix, $for_post_type_prefix, '', false);
		if ( !empty($cfields) ) {
	...
	public static function add_fields_front_candidate($fields) {
		...
		$cfields = self::generate_fields($post_type, $prefix, $for_post_type_prefix, 'profile', false);
		if ( !empty($cfields) ) {
			$fields = array_merge($fields, $cfields);
		}
		return $fields;
	...
	public static function add_fields_front_candidate_resume($fields) {
		$post_type = 'candidate_cfield';
		$prefix = WP_JOB_BOARD_CANDIDATE_CUSTOM_FIELD_PREFIX;
		$for_post_type_prefix = WP_JOB_BOARD_CANDIDATE_PREFIX;

		$cfields = self::generate_fields($post_type, $prefix, $for_post_type_prefix, 'resume', false);
	...
	public static function generate_fields($post_type, $prefix, $for_post_type_prefix, $show_in = '', $admin = true) {
		$custom_fields = self::get_custom_fields($post_type, $show_in);
		...
		foreach ($custom_fields as $post) {
            ...
            if ( !$admin ) {
	            if ( $field_type == 'file' ) {
	            	$cfields[$for_post_type_prefix.$name]['type'] = 'wp_job_board_file';
	            	$cfields[$for_post_type_prefix.$name]['ajax'] = true;
	            	$cfields[$for_post_type_prefix.$name]['file_multiple'] = false;
	            	$cfields[$for_post_type_prefix.$name]['mime_types'] = array( 'gif', 'jpeg', 'jpg', 'png' );
	            } elseif ( $field_type == 'file_list' ) {
	            	$cfields[$for_post_type_prefix.$name]['type'] = 'wp_job_board_file';
	            	$cfields[$for_post_type_prefix.$name]['ajax'] = true;
	            	$cfields[$for_post_type_prefix.$name]['file_multiple'] = true;
	            	$cfields[$for_post_type_prefix.$name]['mime_types'] = array( 'gif', 'jpeg', 'jpg', 'png' );
	            }
            }
