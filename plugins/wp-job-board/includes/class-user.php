...
class WP_Job_Board_User {
	...
	public static function init() {
		...
		// Ajax endpoints.
		add_action( 'wjb_ajax_wp_job_board_ajax_login',  array( __CLASS__, 'process_login' ) );
		add_action( 'wjb_ajax_wp_job_board_ajax_forgotpass',  array( __CLASS__, 'process_forgot_password' ) );
		add_action( 'wjb_ajax_wp_job_board_ajax_register',  array( __CLASS__, 'process_register' ) );

		add_action( 'wjb_ajax_wp_job_board_ajax_change_password',  array(__CLASS__,'process_change_password') );

		// delete profile
		add_action( 'wjb_ajax_wp_job_board_ajax_delete_profile',  array(__CLASS__,'process_delete_profile') );
		// resend approve account
		add_action( 'wjb_ajax_wp_job_board_ajax_resend_approve_account',  array(__CLASS__,'process_resend_approve_account') );
	...
	public static function is_employer($user_id = null) {
	    ...
	    if ($employer_id > 0) {
	    	$employer_id = apply_filters('wp-job-board-post-id', $employer_id, 'employer');
	...
	public static function is_candidate($user_id = 0) {
	    ...
	    if ($candidate_id > 0) {
	    	$candidate_id = apply_filters('wp-job-board-post-id', $candidate_id, 'candidate');
	...
	public static function process_forgot_password() {
		...
		if ( WP_Job_Board_Recaptcha::is_recaptcha_enabled() ) {
			...
			if ( !$is_recaptcha_valid ) {
				$error = esc_html__( 'Captcha is not valid', 'wp-job-board' );
	...
	public static function registration_validation( $username, $email, $password, $confirmpassword, $captcha = true, $check_term = true ) {
		global $reg_errors;
		...
		if ( $check_term ) {
			...
			if ( !empty($page_id) ) {
				if ( empty($_POST['terms_and_conditions']) ) {
					$reg_errors->add('field', esc_html__( 'Terms and Conditions are required', 'wp-job-board' ) );
				}
			}
		}
	...
	public static function registration_save($user_id) {
        ...
        if ($user_role == 'wp_job_board_employer') {
            ...
            $post_args = apply_filters('wp-job-board-create-employer-post-args', $post_args, $user_obj);
            ...
            $post_args = apply_filters('wp-job-board-create-candidate-post-args', $post_args, $user_obj);
