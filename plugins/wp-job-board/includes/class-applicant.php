...
class WP_Job_Board_Applicant {
	...
	public static function init() {
		...
		add_action( 'wjb_ajax_wp_job_board_ajax_applicants_pagination',  array(__CLASS__, 'process_applicants_pagination') );
		// applicants reject
		add_action( 'wjb_ajax_wp_job_board_ajax_reject_applied',  array(__CLASS__, 'process_applicant_reject') );

		// Undo applicants reject
		add_action( 'wjb_ajax_wp_job_board_ajax_undo_reject_applied',  array(__CLASS__, 'process_undo_applicant_reject') );

		// show applicants reject
		add_action( 'wjb_ajax_wp_job_board_ajax_show_rejected_applicants',  array(__CLASS__, 'show_rejected_applicants') );

		// applicants approve
		add_action( 'wjb_ajax_wp_job_board_ajax_approve_applied',  array(__CLASS__, 'process_applicant_approve') );

		// Undo applicants approve
		add_action( 'wjb_ajax_wp_job_board_ajax_undo_approve_applied',  array(__CLASS__, 'process_undo_applicant_approve') );

		// show applicants approve
		add_action( 'wjb_ajax_wp_job_board_ajax_show_approved_applicants',  array(__CLASS__, 'show_approved_applicants') );

		// show applicants
		add_action( 'wjb_ajax_wp_job_board_ajax_show_applicants',  array(__CLASS__, 'show_applicants') );
	...
	public static function process_applicant_reject() {
		...
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_employer($user_id) ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Employer" to reject.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		// send email
		if ( apply_filters('wp-job-board-process-applicant-reject-send-email', true) ) {
			$job = get_post($job_id);
	...
	public static function process_undo_applicant_reject() {
		...
		// send email
		if ( apply_filters('wp-job-board-process-undo-applicant-reject-send-email', true) ) {
			$job = get_post($job_id);
			$candidate_id = self::get_post_meta( $applicant_id, 'candidate_id', true );
			$candidate_name = get_the_title($candidate_id);
			$email_to = WP_Job_Board_Candidate::get_post_meta($candidate_id, 'email', true);
	...
	public static function process_applicant_approve() {
		...
		$user_id = WP_Job_Board_User::get_user_id();
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_employer($user_id) ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Employer" to approve.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		// send email
		if ( apply_filters('wp-job-board-process-applicant-approve-send-email', true) ) {
			$job = get_post($job_id);
			$candidate_id = self::get_post_meta( $applicant_id, 'candidate_id', true );
			$candidate_name = get_the_title($candidate_id);
			$email_to = WP_Job_Board_Candidate::get_post_meta($candidate_id, 'email', true);
		...
		$output = ob_get_clean();
		$return = array( 'status' => true, 'msg' => esc_html__('Candidate approved successful.', 'wp-job-board'), 'output' => $output );
	   	echo wp_json_encode($return);
	   	exit;
	}
	...
	public static function process_undo_applicant_approve() {
		...
		// send email
		if ( apply_filters('wp-job-board-process-undo-applicant-approve-send-email', true) ) {
			$job = get_post($job_id);
			$candidate_id = self::get_post_meta( $applicant_id, 'candidate_id', true );
			$candidate_name = get_the_title($candidate_id);
			$email_to = WP_Job_Board_Candidate::get_post_meta($candidate_id, 'email', true);
		...
		$output = ob_get_clean();
		$return = array( 'status' => true, 'msg' => esc_html__('Undo Candidate approved successful.', 'wp-job-board'), 'output' => $output );
	   	echo wp_json_encode($return);
	   	exit;
	}
	...
	public static function show_applicants() {
		...
		$user_id = WP_Job_Board_User::get_user_id();
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_employer($user_id) ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Employer" to show applications.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
