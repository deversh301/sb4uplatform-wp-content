...
class WP_Job_Board_Mixes {
	...
	public static function filter_field_taxonomy_radio_list($instance, $args, $key, $field) {
		...
		$options = array();
		$query_args = apply_filters('wp-job-board-filter-field-taxonomy-radio-list-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
	...
	public static function filter_field_taxonomy_check_list($instance, $args, $key, $field) {
		...
		$options = array();
		$query_args = apply_filters('wp-job-board-filter-field-taxonomy-check-list-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
		$terms = get_terms($field['taxonomy'], $query_args);
	...
	public static function filter_field_taxonomy_select($instance, $args, $key, $field) {
		...
		$options = array();
		$query_args = apply_filters('wp-job-board-filter-field-taxonomy-taxonomy-select-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
		$terms = get_terms($field['taxonomy'], $query_args);
	...
	public static function hierarchical_tax_tree($catId = 0, $depth = 0, $input_name, $key, $field, $selected, $input_type = 'checkbox'){
	    ...
	    $query_args = apply_filters('wp-job-board-filter-field-taxonomy-tax-tree-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
	...
	public static function hierarchical_tax_option_tree($catId = 0, $depth = 0, $input_name, $key, $field, $selected ){
	    ...
	    $query_args = apply_filters('wp-job-board-filter-field-taxonomy-option-tree-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
	    $query_args['parent'] = $catId;
		$terms = get_terms($field['taxonomy'], $query_args);
	    ...
	    if ( ! is_wp_error( $terms ) && ! empty ( $terms ) ) {
	        foreach ($terms as $term) {
		        ...
		        $output .= '<option value="'.esc_attr($term->term_id).'" '.selected($selected, $term->term_id, false).'>';
	                ...
	                $output .= ' '.sprintf(__('(%d)', 'wp-job-board'), $term->count);
	...
	public static function get_socials_network() {
		return apply_filters( 'wp-job-board-get-socials-network', array(
			...
			'tumblr' => esc_html__('Tumblr', 'wp-job-board'),
			'pinterest' => esc_html__('Pinterest', 'wp-job-board'),
			'instagram' => esc_html__('Instagram', 'wp-job-board'),
			'youtube' => esc_html__('Youtube', 'wp-job-board'),
		));
	}
	...
	public static function get_jobs_page_url() {
		if ( is_post_type_archive('job_listing') ) {
			...
			if ( is_page() && is_object($post) && basename( get_page_template() ) == 'page-jobs.php' ) {
				...
				$jobs_page_id = apply_filters('wp-job-board-post-id', $jobs_page_id, 'page');
				if ( $jobs_page_id ) {
	...
	public static function get_employers_page_url() {
		if ( is_post_type_archive('employer') ) {
			...
			if ( is_page() && is_object($post) && basename( get_page_template() ) == 'page-employers.php' ) {
				...
				$employers_page_id = apply_filters('wp-job-board-post-id', $employers_page_id, 'page');
	...
	public static function get_candidates_page_url() {
		if ( is_post_type_archive('candidate') ) {
			...
			if ( is_page() && is_object($post) && basename( get_page_template() ) == 'page-candidates.php' ) {
				...
				$candidates_page_id = apply_filters('wp-job-board-post-id', $candidates_page_id, 'page');
