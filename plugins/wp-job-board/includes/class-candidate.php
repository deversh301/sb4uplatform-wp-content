...
class WP_Job_Board_Candidate {
	...
	public static function init() {
		...
		// Ajax endpoints.
		add_action( 'wjb_ajax_wp_job_board_ajax_apply_email',  array(__CLASS__,'process_apply_email') );

		// apply job internal
		add_action( 'wjb_ajax_wp_job_board_ajax_apply_internal',  array(__CLASS__,'process_apply_internal') );

		// removed job internal
		add_action( 'wjb_ajax_wp_job_board_ajax_remove_applied',  array(__CLASS__,'process_remove_applied') );

		// add_job_shortlist
		add_action( 'wjb_ajax_wp_job_board_ajax_add_job_shortlist',  array(__CLASS__,'process_add_job_shortlist') );

		// remove job shortlist
		add_action( 'wjb_ajax_wp_job_board_ajax_remove_job_shortlist',  array(__CLASS__,'process_remove_job_shortlist') );

		// wp_job_board_ajax_follow_employer
		add_action( 'wjb_ajax_wp_job_board_ajax_follow_employer',  array(__CLASS__,'process_follow_employer') );

		add_action( 'wjb_ajax_wp_job_board_ajax_unfollow_employer',  array(__CLASS__,'process_unfollow_employer') );

		// download cv
		add_action('wjb_ajax_wp_job_board_ajax_download_file', array( __CLASS__, 'process_download_file' ) );

		// download cv
		add_action('wjb_ajax_wp_job_board_ajax_download_cv', array( __CLASS__, 'process_download_cv' ) );
		
		// download resume cv
		add_action('wjb_ajax_wp_job_board_ajax_download_resume_cv', array( __CLASS__, 'process_download_resume_cv' ) );
	...
	public static function get_salary_html( $post_id = null ) {
		...
		if ( $price_html ) {
			...
			switch ($salary_type) {
				case 'yearly':
					$salary_type_html = esc_html__(' per year', 'wp-job-board');
					break;
				case 'monthly':
					$salary_type_html = esc_html__(' per month', 'wp-job-board');
					break;
				case 'weekly':
					$salary_type_html = esc_html__(' per week', 'wp-job-board');
					break;
				case 'daily':
					$salary_type_html = esc_html__(' per day', 'wp-job-board');
					break;
				case 'hourly':
					$salary_type_html = esc_html__(' per hour', 'wp-job-board');
					break;
				default:
					$all_types = WP_Job_Board_Mixes::get_default_salary_types();
					$salary_type_html = !empty($all_types[$salary_type]) ? $all_types[$salary_type] : $salary_type;
				break;
			}
			$salary_type_html = apply_filters( 'wp-job-board-get-salary-type-html', $salary_type_html, $salary_type, $post_id );
	...
	public static function display_download_cv_btn( $post_id = null, $classes = 'btn btn-theme btn-block btn-download-cv' ) {
		...
		if ( !$check_can_download ) {
			...
			$msg = esc_html__('Please login as "Employer" to download CV.', 'wp-job-board');
		}
	...
	public static function process_apply_email() {
		...
		if ( !$post || empty($post->ID) ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Job doesn\'t exits', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		$result = WP_Job_Board_Email::wp_mail( $author_email, $email_subject, $email_content, $headers, $files_path );
		if ( $result ) {
			$return = array( 'status' => true, 'msg' => esc_html__('You have successfully applied to the job', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		} else {
			$return = array( 'status' => false, 'msg' => esc_html__('Error accord when applying for the job.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
	}
	...
	public static function process_apply_internal() {
		...
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_candidate() ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Candidate" to apply.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		$job_id = !empty($_POST['job_id']) ? $_POST['job_id'] : '';
		...
		if ( !empty($min_percent_resume) && $min_percent_resume > 0 ) {
			...
			if ( $profile_percent < $min_percent_resume ) {
				$return = array( 'status' => false, 'msg' => esc_html__('You need to complete your resume data in order to apply for a job', 'wp-job-board') );
			   	echo wp_json_encode($return);
			   	exit;
			}
		}
        ...
        if ( $applicant_id ) {
	        $return = array( 'status' => true, 'msg' => esc_html__('You have successfully applied to the job', 'wp-job-board'), 'text' => esc_html__('Applied.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
	    } else {
			$return = array( 'status' => false, 'msg' => esc_html__('Error accord when applying for the job', 'wp-job-board') );
	...
	public static function process_remove_applied() {
		...
		if ( empty($applicant_id) ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Applicant doesn\'t exist', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		$user_id = WP_Job_Board_User::get_user_id();
		...
		if ( wp_delete_post( $applicant_id ) ) {
	        $return = array( 'status' => true, 'msg' => esc_html__('Candidate removed successful.', 'wp-job-board') );
	...
	public static function process_add_job_shortlist() {
		...
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_candidate() ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Candidate" to add shortlist.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		$job_id = !empty($_POST['job_id']) ? $_POST['job_id'] : '';
		...
		if ( $result ) {
	        $return = array( 'status' => true, 'nonce' => wp_create_nonce( 'wp-job-board-remove-job-shortlist-nonce' ), 'msg' => esc_html__('Job added to the shortlist successfully.', 'wp-job-board'), 'text' => esc_html__('Shortlisted', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
	    } else {
	...
	public static function process_remove_job_shortlist() {
		...
		if ( $result ) {
	        $return = array( 'status' => true, 'nonce' => wp_create_nonce( 'wp-job-board-add-job-shortlist-nonce' ), 'msg' => esc_html__('Job removed from the shortlist successfully.', 'wp-job-board'), 'text' => esc_html__('Shortlist', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
	    } else {
	...
	public static function process_follow_employer() {
		...
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_candidate() ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Candidate" to follow employer.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		if ( $result ) {
	        $return = array( 'status' => true, 'nonce' => wp_create_nonce( 'wp-job-board-unfollow-employer-nonce' ), 'text' => esc_html__('Following', 'wp-job-board'), 'msg' => sprintf(__('You are now successfully following "%s"', 'wp-job-board'), get_post_field('post_title', $employer_id)) );
		   	echo wp_json_encode($return);
		   	exit;
	    } else {
	...
	public static function process_unfollow_employer() {
		...
		if ( $result ) {
	        $return = array( 'status' => true, 'nonce' => wp_create_nonce( 'wp-job-board-follow-employer-nonce' ), 'text' => esc_html__('Follow us', 'wp-job-board'), 'msg' => sprintf(__('You have successfully unfollowed "%s"', 'wp-job-board'), get_post_field('post_title', $employer_id)) );
		   	echo wp_json_encode($return);
		   	exit;
	    } else {
