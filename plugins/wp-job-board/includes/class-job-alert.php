...
class WP_Job_Board_Job_Alert {
	public static function init() {
		...
		// Ajax endpoints.
		add_action( 'wjb_ajax_wp_job_board_ajax_add_job_alert',  array(__CLASS__,'process_add_job_alert') );

		add_action( 'wjb_ajax_wp_job_board_ajax_remove_job_alert',  array(__CLASS__,'process_remove_job_alert') );
	...
	public static function process_add_job_alert() {
		...
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_candidate() ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Candidate" to add job alert.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
