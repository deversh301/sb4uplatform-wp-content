...
class WP_Job_Board_Employer_Filter extends WP_Job_Board_Abstract_Filter {
	...
	public static function get_fields() {
		...
		$fields['title']	= array(
			'label' => !empty($option['filter_title']) ? $option['filter_title'] : __( 'Search Employer', 'wp-job-board' ),
		...
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields['category'] = array(
				'label' => !empty($option['filter_title']) ? $option['filter_title'] : __( 'Employer Category', 'wp-job-board' ),
		...
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields['center-location'] = array(
				'label' => !empty($option['filter_title']) ? $option['filter_title'] : __( 'Employer Location', 'wp-job-board' ),
		...
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields['location'] = array(
				'label' => !empty($option['filter_title']) ? $option['filter_title'] : __( 'Employer Location List', 'wp-job-board' ),
				'field_call_back' => array( 'WP_Job_Board_Mixes', 'filter_field_taxonomy_hierarchical_check_list'),
		...
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$fields['founded-date'] = array(
				'label' => !empty($option['filter_title']) ? $option['filter_title'] : __( 'Founded Date', 'wp-job-board' ),
		...
		return apply_filters( 'wp-job-board-default-employer-filter-fields', $fields );
	}
	...
	public static function get_query_var_filter($query_vars, $params) {
		...
		if ( ! empty( $params['filter-title'] ) ) {
			...
			$wp_job_board_employer_keyword = sanitize_text_field( wp_unslash($params['filter-title']) );
