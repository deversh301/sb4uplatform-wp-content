...
class WP_Job_Board_Polylang {
	
	public static function init() {
		if ( did_action( 'pll_init' ) ) {

			add_filter( 'wp-job-board-get-custom-fields-key', array(__CLASS__, 'custom_fields_key'), 100, 2 );
			add_filter( 'wp-job-board-get-custom-fields-data', array(__CLASS__, 'get_custom_fields_data'), 100, 2 );

			add_filter( 'wp-job-board-post-id', array(__CLASS__, 'get_post_id'), 10, 2 );
			add_filter( 'wp-job-board-translations-post-ids', array(__CLASS__, 'get_all_translations_object_id'), 10 );
			
			add_filter( 'wp_job_board_settings_job_submission', array(__CLASS__, 'hide_page_selection'), 100 );
			add_filter( 'wp_job_board_settings_pages', array(__CLASS__, 'hide_page_selection'), 100 );
			add_filter( 'wp_job_board_settings_employer_settings', array(__CLASS__, 'hide_page_selection'), 100 );
			add_filter( 'wp_job_board_settings_candidate_settings', array(__CLASS__, 'hide_page_selection'), 100 );
	...
	public static function get_custom_fields_data($value, $prefix) {
		if ( empty($value) ) {
			$value = get_option('wp_job_board_'.$prefix.'_fields_data', array());
		}
		return $value;
	}
	...
	public static function hide_page_selection($fields) {
		...
		foreach ($fields as $key => $field) {
			if ( !empty($field['page-type']) && $field['page-type'] == 'page' ) {
				$fields[$key]['type'] = 'wp_job_board_hidden';
				$fields[$key]['human_value'] = __( 'Page Not Set', 'wp-job-board' );
				...
				$fields[$key]['desc'] = sprintf( __( '<a href="%s">Switch to primary language</a> to edit this setting.', 'wp-job-board' ), $url_to_edit_page );
			}
		}
		return $fields;
	}
