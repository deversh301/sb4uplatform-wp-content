...
class WP_Job_Board_Abstract_Form {
	...
	public function fields_front($metaboxes) {
		...
		$field = array(
			'name'              => !empty($option['title']) ? $option['title'] : __( 'Title', 'wp-job-board' ),
		...
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'      		=> !empty($option['title']) ? $option['title'] : __( 'Types', 'wp-job-board' ),
		...
		$option = wp_job_board_get_option('job_field_description');
		$option = isset($option[0]) ? $option[0] : array();
		$field = array(
		...
		$option = wp_job_board_get_option('job_field_featured_image');
		$option = isset($option[0]) ? $option[0] : array();
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
		...
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : sprintf(__( 'Min. Salary (%s)', 'wp-job-board' ), $currency_symbol),
		...
		if ( empty($option['enable']) || $option['enable'] == 'on' ) {
			$field = array(
				'name'              => !empty($option['title']) ? $option['title'] : sprintf(__( 'Max. Salary (%s)', 'wp-job-board' ), $currency_symbol),
		...
		$fields = apply_filters( 'wp-job-board-job_listing-fields-front', $fields);
