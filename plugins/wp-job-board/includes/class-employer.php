...
class WP_Job_Board_Employer {
	...
	public static function init() {
		// Ajax endpoints.
		add_action( 'wjb_ajax_wp_job_board_ajax_employer_add_employee', array( __CLASS__, 'add_employee' ) );
		add_action( 'wjb_ajax_wp_job_board_ajax_employer_remove_employee', array( __CLASS__, 'remove_employee' ) );

		// add_job_shortlist
		add_action( 'wjb_ajax_wp_job_board_ajax_add_candidate_shortlist',  array(__CLASS__,'process_add_candidate_shortlist') );

		// remove job shortlist
		add_action( 'wjb_ajax_wp_job_board_ajax_remove_candidate_shortlist',  array(__CLASS__,'process_remove_candidate_shortlist') );
	...
	public static function add_employee() {
		...
		// add employee user
		WP_Job_Board_User::registration_validation( $_POST['username'], $_POST['email'], $_POST['password'], $_POST['confirmpassword'], false, false );
	...
	public static function process_add_candidate_shortlist() {
		...
		if ( !is_user_logged_in() || !WP_Job_Board_User::is_employer() ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Please login as "Employer" to add shortlist.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		if ( !$post || empty($post->ID) ) {
			$return = array( 'status' => false, 'msg' => esc_html__('Candidate doesn\'t exist.', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
		}
		...
		if ( $result ) {
	        ...
	        $return = array( 'status' => true, 'nonce' => wp_create_nonce( 'wp-job-board-remove-candidate-shortlist-nonce' ), 'msg' => esc_html__('Candidate has been added to the shortlist successfully', 'wp-job-board'), 'text' => esc_html__('Shortlisted', 'wp-job-board') );
		   	echo wp_json_encode($return);
		   	exit;
	    } else {
