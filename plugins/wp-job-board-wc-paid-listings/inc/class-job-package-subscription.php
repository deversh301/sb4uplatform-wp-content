...
class WP_Job_Board_Wc_Paid_Listings_Job_Package_Subscription {
			...
			if ( $post_type === 'job_listing' ) {
				...
				if ( $package_product_id ) {
					...
					if ( 'listing' === $subscription_type ) {
						$new_count = get_post_meta($user_package_id, WP_JOB_BOARD_WC_PAID_LISTINGS_PREFIX.'package_count', true);
						$new_count --;

						update_post_meta($user_package_id, WP_JOB_BOARD_WC_PAID_LISTINGS_PREFIX.'package_count', $new_count);
					}
				}
			}
		}
			...
			if ( 'job_listing' === $post_type ) {
				...
				if ( $package_product_id ) {
					...
					if ( 'listing' === $subscription_type ) {
						$new_count = get_post_meta($user_package_id, WP_JOB_BOARD_WC_PAID_LISTINGS_PREFIX.'package_count', true);
						$new_count++;
						$job_limit = get_post_meta($user_package_id, WP_JOB_BOARD_WC_PAID_LISTINGS_PREFIX.'job_limit', true);
	...
	public static function subscription_ended( $subscription ) {
			...
			if ( $user_packages ) {
				foreach ($user_packages as $user_package_id) {
					...
					if ( $package_type == 'job_package' ) {
						...
						if ( 'listing' === $subscription_type ) {
							$listing_ids = WP_Job_Board_Wc_Paid_Listings_Mixes::get_listings_for_package( $user_package_id, 'job_listing' );
						...
						// Expire listings posted with package
						if ( 'listing' === $subscription_type ) {
							$listing_ids = WP_Job_Board_Wc_Paid_Listings_Mixes::get_listings_for_package( $user_package_id, 'candidate' );
		...
		delete_post_meta( $subscription->get_id(), 'wp_job_board_wc_paid_listings_packages_processed' );
	}
	...
	public static function subscription_activated( $subscription ) {
		...
		if ( get_post_meta( $subscription->get_id(), 'wp_job_board_wc_paid_listings_packages_processed', true ) ) {
			return;
		}
		...
		foreach ( $subscription->get_items() as $item ) {
			...
			if ( $product->is_type( array( 'job_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
				...
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
				}
				...
				foreach ( $listing_ids as $listing_id ) {
					if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ) ) ) {
						WP_Job_Board_Wc_Paid_Listings_Mixes::approve_job_with_package( $listing_id, $subscription->get_user_id(), $user_package_id );
				...
				// Give packages to user
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_resume_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
				}

				WP_Job_Board_Wc_Paid_Listings_Mixes::increase_expiry_with_package( $subscription->get_user_id(), $user_package_id );
				$candidate_id = WP_Job_Board_User::get_candidate_by_user_id($subscription->get_user_id());
				if ( $candidate_id ) {
					update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'package_id', $product->get_id() );
				}

			} elseif ( $product->is_type( array( 'cv_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
				// Give packages to user
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_cv_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
				}
				...
				// Give packages to user
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_contact_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
				}
				...
				// Give packages to user
				for ( $i = 0; $i < $item['qty']; $i ++ ) {
					$user_package_id = WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_candidate_package( $subscription->get_user_id(), $product->get_id(), $subscription->get_id() );
				}
			}
		}

		update_post_meta( $subscription->get_id(), 'wp_job_board_wc_paid_listings_packages_processed', true );
	}
			...
			if ( 'package' === $subscription_type ) {
				if ( $user_packages ) {
					...
					if ( $product->get_type() == 'job_package_subscription' ) {
						WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_package( $subscription->get_user_id(), $item['product_id'], $subscription->get_id() );
					} elseif ( $product->get_type() == 'cv_package_subscription' ) {
						WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_cv_package( $subscription->get_user_id(), $item['product_id'], $subscription->get_id() );
					} elseif ( $product->get_type() == 'contact_package_subscription' ) {
						WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_contact_package( $subscription->get_user_id(), $item['product_id'], $subscription->get_id() );
					} elseif ( $product->get_type() == 'candidate_package_subscription' ) {
						WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_candidate_package( $subscription->get_user_id(), $item['product_id'], $subscription->get_id() );
					} elseif ( $product->get_type() == 'resume_package_subscription' ) {
						WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_resume_package( $subscription->get_user_id(), $item['product_id'], $subscription->get_id() );
					}
				}
				...
				if ( $user_packages ) {
					foreach ( $user_packages as $user_package_id ) {
						...
						if ( $package_type == 'job_package' ) {
							...
							if ( $listing_ids = WP_Job_Board_Wc_Paid_Listings_Mixes::get_listings_for_package( $user_package_id, 'job_listing' ) ) {
								foreach ( $listing_ids as $listing_id ) {
									// Featured | Urgent or not
									update_post_meta( $listing_id, WP_JOB_BOARD_JOB_LISTING_PREFIX. 'urgent', $urgent );
									update_post_meta( $listing_id, WP_JOB_BOARD_JOB_LISTING_PREFIX. 'featured', $featured );
								}
							}
						} elseif ( $package_type == 'resume_package' ) {
							...
							$candidate_id = WP_Job_Board_User::get_candidate_by_user_id($subscription->get_user_id());
							if ( $candidate_id ) {
								update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'urgent', $urgent );
								update_post_meta( $candidate_id, WP_JOB_BOARD_CANDIDATE_PREFIX.'featured', $featured );
							}
						} elseif ( $package_type == 'cv_package' ) {
							update_post_meta($user_package_id, $prefix.'cv_viewed_count', '');
							update_post_meta($user_package_id, $prefix.'cv_viewed_count_nb', 0);
		...
		if ( $user_packages ) {
			...
			foreach ($user_packages as $user_package_id) {
				...
				if ( $package_type == 'job_package' ) {
					...
					$switching_to_package_id = WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_package( $user_id, $new_subscription->product_id, $new_subscription->id );
					...
					if ( 'listing' === $new_subscription->type && $switching_to_package_id ) {
						...
						foreach ( $listing_ids as $listing_id ) {
							...
							if ( ! $is_upgrade ) {
								...
								WP_Job_Board_Wc_Paid_Listings_Mixes::increase_package_count( $user_id, $switching_to_package_id );
								// Change the user package ID and package ID
								update_post_meta( $listing_id, WP_JOB_BOARD_JOB_LISTING_PREFIX.'user_package_id', $switching_to_package_id );
								update_post_meta( $listing_id, WP_JOB_BOARD_JOB_LISTING_PREFIX.'package_id', $new_subscription->product_id );
							}
					...
					// Give new package to user
					$switching_to_package_id = WP_Job_Board_Wc_Paid_Listings_Mixes::create_user_resume_package( $user_id, $new_subscription->product_id, $new_subscription->id );
					// Delete the old package
					wp_delete_post($user_package_id);
					...
					if ( 'listing' === $new_subscription->type && $switching_to_package_id ) {
						...
						if ( $candidate_id ) {
							// Featured | Urgent or not
							update_post_meta( $candidate_id, WP_JOB_BOARD_JOB_LISTING_PREFIX.'urgent', $urgent );
							update_post_meta( $candidate_id, WP_JOB_BOARD_JOB_LISTING_PREFIX.'featured', $featured );
							// Fire action
...
WP_Job_Board_Wc_Paid_Listings_Job_Package_Subscription::init();