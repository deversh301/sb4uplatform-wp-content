...
<article id="post-<?php echo esc_attr($post->ID); ?>" <?php post_class('employer-single-v5'); ?>>
	<!-- heading -->
	<?php
	$cover_photo = get_post_meta( $post->ID, WP_JOB_BOARD_EMPLOYER_PREFIX . 'cover_photo', true );
	$style = '';
	if ( $cover_photo ) {
	    $style = 'style="background:url('.esc_url($cover_photo).')"';
	}
	?>
	<div class="employer-detail-header-wrapper" <?php echo trim($style); ?>>
		<div class="container">
			<?php echo WP_Job_Board_Template_Loader::get_template_part( 'single-employer/header-v4' ); ?>
		</div>
	</div>
	...
	<div class="container">
		...
		<div class="row content-job-detail">
			<div class="left-content-single-employer col-xs-12 col-md-<?php echo esc_attr( is_active_sidebar( 'employer-single-sidebar' ) ? 8 : 12); ?>">
