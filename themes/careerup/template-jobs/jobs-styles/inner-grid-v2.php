...
<article id="post-<?php the_ID(); ?>" <?php post_class('job-grid style3 text-center job-card'); ?> data-latitude="<?php echo esc_attr($latitude); ?>" data-longitude="<?php echo esc_attr($longitude); ?>">
    <?php careerup_job_display_featured_urgent($post); ?>
	<?php if ( has_post_thumbnail($employer_id) ) { ?>
        <div class="employer-logo justify-content-center flex-middle">
