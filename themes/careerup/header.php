...
<body <?php body_class(); ?>>
<?php if ( careerup_get_config('preload', true) ) {
	$preload_icon = careerup_get_config('media-preload-icon');
...
<div id="wrapper-container" class="wrapper-container">
	...
	<?php
		$header = apply_filters( 'careerup_get_header_layout', careerup_get_config('header_type') );
		get_template_part( 'headers/'.$header );
		if ( !empty($header) ) {
			careerup_display_header_builder($header);
		} else {
