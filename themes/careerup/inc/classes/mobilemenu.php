...
if ( !class_exists("Careerup_Mobile_Menu") ) {
    class Careerup_Mobile_Menu extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'careerup');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'careerup');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'careerup');
                        break;
        ...
        public function generateSubMegaMenu( $item, $apus_mega_profile ) {
            if ( $apus_mega_profile ) {
                ...
                if ( $post_ids && isset($post_ids[0]) ) {
                    $post_id = $post_ids[0];
                    $post_id = careerup_get_lang_post_id($post_id, 'apus_megamenu');
                    $post = get_post($post_id);

                    $content = apply_filters( 'careerup_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
