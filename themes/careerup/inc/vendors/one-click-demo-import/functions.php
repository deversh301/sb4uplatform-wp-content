...
function careerup_ocdi_import_files() {
    $demos = array();
    for ($i=1; $i <= 13; $i++) {
        $demos[] = array(
            ...
            'local_import_redux'           => array(
                array(
                    ...
                    'option_name' => 'careerup_theme_options',
                ),
            ),
            'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'inc/vendors/one-click-demo-import/default/screenshot'.$i.'.png',
            'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'careerup' ),
            'preview_url'                  => 'https://apusthemes.com/wp-demo/careerup/',
        );
    }
    return apply_filters( 'careerup_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'careerup_ocdi_import_files' );

function careerup_ocdi_after_import_setup( $selected_import ) {
    ...
    if ( file_exists($file) ) {
        careerup_ocdi_import_settings($file);
    }
...
add_action( 'pt-ocdi/after_import', 'careerup_ocdi_after_import_setup' );

function careerup_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        careerup_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        careerup_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function careerup_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function careerup_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function careerup_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'careerup_ocdi_before_widgets_import' );