...
class Careerup_Elementor_Job_Board_Search_Form2 extends Elementor\Widget_Base {

    public function get_name() {
        return 'careerup_job_board_search_form2';
    }

    public function get_title() {
        return esc_html__( 'Apus Jobs Search Form 2', 'careerup' );
    }
    ...
    public function get_form_fields() {
        ...
        return array(
            ...
            'type' => array(
                'label' => esc_html__( 'Job Type', 'careerup' ),
                'field_call_back' => array( 'WP_Job_Board_Mixes', 'filter_field_taxonomy_hierarchical_select'),
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    // Stronger selector to avoid section style from overwriting
                    '{{WRAPPER}} .form-group' => 'background-color: {{VALUE}};',
                ],
            ]
        );
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    // Stronger selector to avoid section style from overwriting
                    '{{WRAPPER}} .form-group' => 'border-color: {{VALUE}} !important;',
                ],
            ]
        );
    ...
    protected function render() {
        ...
        <div class="widget-job-search-form <?php echo esc_attr($el_class); ?>">
            ...
            <form action="<?php echo esc_url($search_page_url); ?>" class="form-search" method="GET">
                <div class="flex-middle-sm search-form-inner search-form-inner-v2 <?php echo esc_attr($style); ?>">
                    <div class="row">
                        <?php
                        if ( !empty($main_search_fields) ) {
                            ...
                            foreach ($main_search_fields as $item) {
                                ...
                                ?>
                                <div class="col-xs-12 col-md-<?php echo esc_attr($columns); ?> <?php echo esc_attr( $count < count($main_search_fields) )?'has-border':'' ?>">
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Careerup_Elementor_Job_Board_Search_Form2 );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Careerup_Elementor_Job_Board_Search_Form2 );
}