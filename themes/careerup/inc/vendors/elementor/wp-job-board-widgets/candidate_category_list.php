...
class Careerup_Elementor_Job_Board_Candidate_Category_List extends Elementor\Widget_Base {

	public function get_name() {
        return 'apus_element_job_board_candidate_category_list';
    }

	public function get_title() {
        return esc_html__( 'Apus Candidate Category List', 'careerup' );
    }
                            ...
                            <div class="catgory-parent col-md-3 col-xs-12">
                                <a href="<?php echo esc_url(get_term_link( $term, 'candidate_category' )); ?>">
                                    ...
                                    <?php if ( $show_nb_jobs ) {
                                        ...
                                        $query = careerup_get_candidates($args);
                                        $number_jobs = $count = $query->found_posts;
                                        $number_jobs = $number_jobs ? WP_Job_Board_Mixes::format_number($number_jobs) : 0;
                                    ?>
                                        <div class="number_total">(<?php echo sprintf(_n('<span>%d</span> Candidate', '<span>%d</span> Candidates', $count, 'careerup'), $number_jobs); ?>)</div>
                                    <?php } ?>
                                </a>
                            </div>
                                                    ...
                                                    <li>
                                                        <a href="<?php echo esc_url(get_term_link( $term, 'candidate_category' )); ?>">
                                                            ...
                                                            <?php if ( $show_nb_jobs ) {
                                                                ...
                                                                $number_jobs = $number_jobs ? WP_Job_Board_Mixes::format_number($number_jobs) : 0;
                                                            ?>
                                                                <span class="number"> (<?php echo sprintf(_n('<span>%d</span> Candidate', '<span>%d</span> Candidates', $count, 'careerup'), $number_jobs); ?>)</span>
                                                            <?php } ?>
                                                            </div>
                                                        </a>
                                                    </li>
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Careerup_Elementor_Job_Board_Candidate_Category_List );
} else {
    Elementor\Plugin::instance()->widgets_manager->register( new Careerup_Elementor_Job_Board_Candidate_Category_List );
}